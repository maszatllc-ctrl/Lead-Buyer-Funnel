...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Send lead data to webhook first (independent of FB credentials)
    if (WEBHOOK_URL) {
      try {
    ...
    // Send to FB Conversions API (only if credentials are configured)
    let fbResult = null;
    if (PIXEL_ID && ACCESS_TOKEN) {
      ...
      console.warn('Missing FB_PIXEL_ID or FB_ACCESS_TOKEN — skipping FB CAPI, webhook still sent');
    }
